...
namespace egn {
    ...
    inline void drawTriangle(const egn::ShadedVertex& v0, const egn::ShadedVertex& v1, const egn::ShadedVertex& v2, 
        ...
        for (int x = minX; x <= maxX; ++x) {
            for (int y = minY; y <= maxY; ++y) {
                ...
                float w0 = (v2.screenPos.x - v1.screenPos.x) * (py - v1.screenPos.y) - (v2.screenPos.y - v1.screenPos.y) * (px - v1.screenPos.x);
                float w1 = (v0.screenPos.x - v2.screenPos.x) * (py - v2.screenPos.y) - (v0.screenPos.y - v2.screenPos.y) * (px - v2.screenPos.x);
                float w2 = (v1.screenPos.x - v0.screenPos.x) * (py - v0.screenPos.y) - (v1.screenPos.y - v0.screenPos.y) * (px - v0.screenPos.x);
                ...
                if (inside) {
                    ...
                    if (z < fb.getDepth(x, y)) {
                        ...
                        float u = (alpha * v0.uv.x + beta * v1.uv.x + gamma * v2.uv.x) / invW;
                        float v = (alpha * v0.uv.y + beta * v1.uv.y + gamma * v2.uv.y) / invW;
                        ...
                        if (light) {
                            ...
                            // Diffuse
                            float diff = std::max(dot_product(normal, lightDir), 0.0f);

                            // Specular (Blinn-Phong)
                            gll::Vec3 halfway = (lightDir + viewDir).normalized();
                            float spec = std::pow(std::max(dot_product(normal, halfway), 0.0f), light->shininess);

                            float lighting = light->ambient + light->diffuse * diff + light->specular * spec;
                            lighting = std::min(lighting, 1.0f);
